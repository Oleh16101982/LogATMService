		...
		public fWriteEventLog(String fLogName, String fSource, String fMsgFile)
		{
			if (!EventLog.SourceExists(fSource))
			{
				...
				if (!System.IO.File.Exists(fMsgFile))
				{
					...
					throw new LogATMClient.cliServiceExceptions.EMsgFileNotFoundException("Message file for EventLog - " + fMsgFile + " not found", 0x01);
				}
				...
				throw new LogATMClient.cliServiceExceptions.ENeedServiceRestartException("Restart service neededd.", 0x01);
			}
			LogName = fLogName;
			SrcName = fSource;		
		}
